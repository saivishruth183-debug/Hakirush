...
import { 
  ShoppingBag, CheckCircle, ArrowRight, Sparkles, ArrowLeft, Trophy, 
  Shirt, Gift, PenTool, Zap, Activity, CircleDot 
} from "lucide-react";
import { useNavigate } from "react-router-dom";

// --- BACKGROUND SUB-COMPONENT ---
const ContinuousSportsBackground = () => {
  // Theme-specific icons for Merchandise & Branding
  const row1 = [ShoppingBag, Shirt, Gift, PenTool, Zap, CircleDot];
  const row2 = [Trophy, Shirt, ShoppingBag, Activity, Gift, Sparkles];
  ...
  return (
    <div className="fixed inset-0 overflow-hidden bg-white pointer-events-none z-0">
      ...
      {/* Row 1: Moving Left */}
      <div className="flex absolute top-[12%] opacity-[0.04]">
        <motion.div 
          initial={{ x: 0 }}
          animate={{ x: "-100%" }}
        ...
        >
          {row1.map((Icon, i) => <Icon key={i} size={82} className="text-red-900" strokeWidth={1} />)}
          {row1.map((Icon, i) => <Icon key={`dup-${i}`} size={82} className="text-red-900" strokeWidth={1} />)}
        </motion.div>
      </div>

      {/* Row 2: Moving Right */}
      <div className="flex absolute top-[58%] opacity-[0.03]">
        <motion.div 
          initial={{ x: "-100%" }}
          animate={{ x: 0 }}
        ...
        >
          {row2.map((Icon, i) => <Icon key={i} size={112} className="text-red-900" strokeWidth={0.5} />)}
          {row2.map((Icon, i) => <Icon key={`dup2-${i}`} size={112} className="text-red-900" strokeWidth={0.5} />)}
        </motion.div>
      </div>
...
const SPONSORS = [
  {
    ...
    details: {
      ...
      whatYouGet: [
        ...
        "Branded packaging for all winner kits and gift hampers",
      ],
      why: [
        ...
        <div className="max-w-6xl mx-auto px-6 relative z-10">
          <div className="grid md:grid-cols-2 gap-10">
            ...
            >
              <div className="relative p-10">
                <div className="inline-flex items-center gap-1.5 px-3 py-2 sm:px-5 sm:py-2.5 rounded-full bg-gradient-to-r from-[#C21807] to-[#A01506] text-white mb-8 shadow-lg group-hover:shadow-[#C21807]/50 transition-shadow duration-300">
                  <Shirt className="w-4 h-4 animate-pulse" />
