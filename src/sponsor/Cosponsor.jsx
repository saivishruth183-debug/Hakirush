...
import { 
  Users, CheckCircle, ArrowRight, Sparkles, ArrowLeft, Trophy, Handshake,
  Activity, Target, Zap, CircleDot, Flag 
} from "lucide-react";
import { useNavigate } from "react-router-dom";

// --- BACKGROUND SUB-COMPONENT ---
const ContinuousSportsBackground = () => {
  // Theme-specific icons for Co-Sponsorship
  const row1 = [Handshake, Activity, Users, CircleDot, Target, Zap];
  const row2 = [Flag, Users, Trophy, Handshake, Target, Sparkles];
  ...
  return (
    <div className="fixed inset-0 overflow-hidden bg-white pointer-events-none z-0">
      ...
      {/* Row 1: Moving Left */}
      <div className="flex absolute top-[15%] opacity-[0.04]">
        <motion.div 
          initial={{ x: 0 }}
          animate={{ x: "-100%" }}
        ...
        >
          {row1.map((Icon, i) => <Icon key={i} size={85} className="text-red-900" strokeWidth={1} />)}
          {row1.map((Icon, i) => <Icon key={`dup-${i}`} size={85} className="text-red-900" strokeWidth={1} />)}
        </motion.div>
      </div>

      {/* Row 2: Moving Right */}
      <div className="flex absolute top-[55%] opacity-[0.03]">
        <motion.div 
          initial={{ x: "-100%" }}
          animate={{ x: 0 }}
        ...
        >
          {row2.map((Icon, i) => <Icon key={i} size={115} className="text-red-900" strokeWidth={0.5} />)}
          {row2.map((Icon, i) => <Icon key={`dup2-${i}`} size={115} className="text-red-900" strokeWidth={0.5} />)}
        </motion.div>
      </div>
...
export default function SponsorGrid() {
  ...
  return (
    <div className="relative overflow-hidden bg-gray-50 min-h-screen">
      ...
      {/* Back Button - z-20 to ensure clickability */}
