...
const About = () => {
  return (
    <section className="relative py-10 bg-[#FFF5F5] overflow-hidden">

      <div className="absolute inset-0 bg-gradient-to-br from-[#C21807]/15 via-transparent to-transparent blur-3xl -z-10"></div>

      {/* MAIN CONTENT */}
      <div className="max-w-7xl mx-auto px-4 sm:px-6 md:px-10 lg:px-6 grid grid-cols-1 md:grid-cols-2 gap-12 md:gap-20 items-center">
        ...
        <motion.div
          ...
          className="space-y-5"
        >
          <h2 className="text-3xl sm:text-4xl md:text-5xl font-extrabold text-[#C21807]">
            Who We Are
          </h2>
        ...
        <motion.div
          ...
          transition={{ duration: 0.9 }}
          viewport={{ once: true }}
          className="grid grid-cols-2 gap-5 max-w-sm sm:max-w-none mx-auto"
        >
          {aboutimg.map((item, index) => (
            <motion.div
              ...
              viewport={{ once: true }}
              className="rounded-xl overflow-hidden shadow-md hover:scale-[1.04] transition"
            >
              <img
                src={item.image}
      ...
      {/* VISION & MISSION */}
      <div className="mt-14 px-4 sm:px-6 flex flex-col md:flex-row justify-center gap-6 md:gap-14">
        {about.map((item,index)=>(
          <motion.div
            ...
            viewport={{ once: true }}
            className="w-full md:w-1/3 bg-white shadow-lg px-7 sm:px-10 py-10 sm:py-14 rounded-2xl hover:shadow-red-400 transition"
          >
            <h3 className="flex items-center gap-4 text-xl sm:text-2xl font-extrabold mb-4 text-black">
              <span className='p-3 bg-red-200 rounded'>{item.icon}</span> 
              {item.title}
            </h3>
      ...
      {/* STATS */}
      <div className="mt-16 px-4 sm:px-6 grid grid-cols-1 sm:grid-cols-3 gap-8 text-center max-w-6xl mx-auto">
        {[
          ...
          <motion.div
            ...
            transition={{ type: 'spring', stiffness: 200 }}
            viewport={{ once: true }}
            className="bg-white rounded-xl shadow-md border border-gray-100 py-10 hover:shadow-red-400 transition"
          >
            <div className="text-4xl mb-2">{stat.icon}</div>
            <div className="text-3xl font-bold text-[#C21807]">{stat.value}+</div>
